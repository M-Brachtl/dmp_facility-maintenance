...
export class PlansComponent {
  ...
  getMonthName(monthIndex: string): string {
    const date = new Date();
    date.setMonth(Number(monthIndex) - 1);
    return new Intl.DateTimeFormat('cs-CZ', { month: 'long' }).format(date);
  }
  ...
  limitGlobal = 2;
  markedWeeks: Set<string> = new Set();
  markOverflowWeeks(limit: number = this.limitGlobal, nextMonth: boolean = false): void { // limit = max number of planned revs per week
    const calendarToUse = nextMonth ? this.next_month_cal : this.calendar;
    this.markedWeeks.clear();
    const weekCount: { [weekID: string]: number } = {}; // weekID = yyyy-ww (ex: 2025-11) meaning week 11 of year 2025
    for (const key in calendarToUse) {
      calendarToUse[key].machines.forEach(entry => {
        const weekNum = `${key.split('-')[0]}-${this.getISOWeekNumber(entry[0])}`;
        // console.log('WeekNum:', weekNum);
        weekCount[weekNum] = (weekCount[weekNum] || 0) + 1;
      });
    }
    for (const weekNum in weekCount) {
      // console.log('Week:', weekNum, 'Count:', weekCount[weekNum]);
      if (weekCount[weekNum] > limit) {
        this.markedWeeks.add(weekNum);
      }
    }
    console.log('Marked Weeks:', this.markedWeeks);
  }
  inMarkedWeek(date: any): boolean {
    const weekNum = `${date.getFullYear()}-${this.getISOWeekNumber(date)}`;
    // console.log('Checking weekNum:', weekNum, 'MarkedWeeks:', this.markedWeeks, 'Result:', this.markedWeeks.has(weekNum));
    return this.markedWeeks.has(weekNum);
  }
  weekKey(date: any): string {
    return `${date.getFullYear()}-${this.getISOWeekNumber(date)}`;
  }
...
export async function allCalendarMachines(eel_on: boolean): Promise<{ [key: string]: { machines: (Date | string | boolean)[][]; people: (Date | string)[][] } }> {
  let current_plan: { title: string; active: boolean; machines: any[]; people: any[] } = { title: '', active: false, machines: [], people: [] };
  if (eel_on) {
    await new Promise<void>(resolve => {
      eel.generate_plan_api()((response: { "status": string, "plan": any }) => {
